...
const FundamentalQuizPage = () => {
  ...
  // Initialize showNav based on window width after component mounts
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        for (const type of typesToLoad) {
          try {
            // Load from fundamentals folder structure
            const module = await import(
              `@/data/fundamentals/subjects/${subjectId}/${levelId}/${type}.json`
            );
            
            // Check if the module has questions or if we need to use the root level
            const questions = module.questions || module;
            
            if (questions && (Array.isArray(questions) ? questions.length > 0 : Object.keys(questions).length > 0)) {
              questionsByType[type] = (Array.isArray(questions) ? questions : [questions]).map((q: any) => ({
                ...q,
        ...
        // If there's an error, redirect back to fundamentals page after a delay
        setTimeout(() => {
  ...
  }, [subjectId, levelId, questionType, questionCount, router]);
...
export default FundamentalQuizPage;